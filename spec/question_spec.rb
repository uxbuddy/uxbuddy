describe Question do
  ...
  describe "Ave method" do

    xit 'should calculate the average of the responses' do
      expect(question1.ave_response(test.id)).to eq(average.round(1))
    end

    it "should return 'unanswered' if there are no responses" do
      question2.test_types << TestType.where(id: 1)
      expect(question2.ave_response(test2.id)).to eq("Unanswered")
    end
  ...
  describe "Display comments" do
    ...
    it 'should display all comments for particular question' do
      expect(question1.comments(test.id)).to eq [comment1, comment2, comment3, 'This is so boss']
    end
